...
#### 七大设计原则
1. 单一职责原则（SRP）：一个类应该有且只赋予一个职责。
   + 单一功能
   + 解耦和增强内聚性（高内聚，低耦合）
   
2. 开闭原则（OCP）：对扩展开放，对修改关闭。
   
3. 里氏替换原则（LSP）：派生类型必须可以完全替换其基类型。
   + 在任何父类出现的地方都可以用他的子类来替代。
   + 子类可以有自己的个性。
   + 覆盖或实现父类的方法时输入参数可以被放大，输出结果可以被缩小。
   
4. 接口隔离原则（ISP）：不应强迫客户实现对自身无用的方法。
...
### 其他
+ 混沌工程：是在分布式系统上进行实验的学科, 目的是建立对系统抵御生产环境中失控条件的能力以及信心。最早由Netflix及相关团队提出。
...
+ BCDE原则：
  ...
  + E（error）强制错误信息输入（如：非法数据、异常流程、业务允许外等）得到预期结果。
